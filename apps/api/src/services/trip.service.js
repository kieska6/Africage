...
class TripService {
    ...
    const trip = await prisma.trip.findUnique({
      ...
      include: {
        transactions: {
          select: { 
            status: true, 
            shipment: { select: { weight: true, length: true, width: true, height: true } } 
          }
        }
      }
    });
  ...
  async canModifyTrip(tripId, userId) {
    ...
    // Si le trajet a des transactions actives, seuls certains champs peuvent être modifiés
    if (trip.transactions.length > 0) {
      return { limited: true }; // Modification limitée
    }

    return { full: true }; // Modification complète
  }
}
